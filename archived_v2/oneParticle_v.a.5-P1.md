...
```python
tPiTest = np.append(np.arange(0.06,0,-2*dt), 0) # note this is decending
...
```python editable=true slideshow={"slide_type": ""}
plt.imshow(np.fliplr(np.flipud(vtSliceM1)),aspect=aspect=2.5*len(tPiTest)/len(intensityScan), 
