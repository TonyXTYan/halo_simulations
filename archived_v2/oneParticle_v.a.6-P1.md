...
```python
tPiTest = np.append(np.arange(0.06,0,-3*dt), 0) # note this is decending
...
```python editable=true slideshow={"slide_type": ""}
intensityScan = np.arange(0.04,2.02,0.02)
...
```python editable=true slideshow={"slide_type": ""}
plt.imshow(np.fliplr(np.flipud(vtSliceM1)),aspect=10*len(tPiTest)/len(intensityScan), 
...
plt.title(title)
plt.savefig(output_prefix+"Transfer fraction of halo into.pdf", dpi=600)
plt.savefig(output_prefix+"Transfer fraction of halo into.png", dpi=600)
plt.show()
```
