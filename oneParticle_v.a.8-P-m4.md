...
expPGrid4 = np.zeros((nx,nz),dtype=complex)
for indx in range(nx):
    expPGrid4[indx, :] = np.exp(-(1j/hb) * (0.5/m4) * (dt) * (pxlin[indx]**2 + pzlin**2))  
```
...
tPiOutput0d8 = Parallel(n_jobs=N_JOBS)(
    delayed(lambda i: (i, scanTauPiInnerEval(i, False, False,0,p,0*dopd,0.8*VR)[:2]) )(i) 
    for i in tqdm(tPiTest)
) 
<!-- #endraw -->

<!-- #raw -->
tPiOutput0d9 = Parallel(n_jobs=N_JOBS)(
    delayed(lambda i: (i, scanTauPiInnerEval(i, False, False,0,p,0*dopd,0.9*VR)[:2]) )(i) 
    for i in tqdm(tPiTest)
) 
<!-- #endraw -->
